...
### 2026-07-27
- **新增**：多小区权限控制 —— 主管新建/编辑小区时可勾选哪些师傅/管家有权访问；非主管角色只能看到自己被授权的小区（+ 默认小区）
- **新增**："我的"页面 —— 师傅/管家的"师傅日程"改为"我的"，包含个人日程表 + 状态切换（在岗待命/正在处理/请假），状态变更实时同步到后端，主管管理平台可见
- **优化**：状态标签从"在岗/忙碌/休息"改为"在岗待命/正在处理/请假"，语义更明确；状态根据工单自动推导（有处理中工单 → 正在处理，无则在岗待命，手动请假不被覆盖）
- **优化**：师傅日程改为竖版布局 —— X轴为师傅名字（列），Y轴为 0:00-24:00 时间段（从上到下），工单块竖向排列
- **权限**：师傅/管家角色的日程页面只能查看自己的排班，不可切换查看其他人
- **清理**：删除 app.js 中约 380 行旧版重复函数定义，仅保留增强版
